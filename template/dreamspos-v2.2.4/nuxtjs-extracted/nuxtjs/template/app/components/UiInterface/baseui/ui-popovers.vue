...
export default {
    ...
    setup() {
        ...
        const initializePopovers = async () => {
            // Try to get Popover from window.bootstrap first (from bundle import)
            if (window.bootstrap && window.bootstrap.Popover) {
                Popover = window.bootstrap.Popover;
            } else {
                // Fallback: dynamically import Popover
                try {
                    const bootstrap = await import('bootstrap');
                    Popover = bootstrap.Popover;
                } catch (error) {
                    console.error('Failed to load Bootstrap Popover:', error);
                    return;
                }
            }
            ...
            popoverElements.forEach(element => {
                ...
                if (Popover.getInstance(element)) {
                    return;
                }
                ...
                const options = {
                    ...
                    title: element.getAttribute('title') || element.getAttribute('data-bs-original-title') || '',
