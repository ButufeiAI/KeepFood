...
import { payment, apiResultFormat } from '../../../../core/models/model';
import {
  pageSelection,
  PaginationService,
  tablePageSize,
...
export class Payments {
  ...
  tableData = signal<payment[]>([]);
  tableDataCopy = signal<payment[]>([]);
  allData: WritableSignal<payment[]> = signal([]);
  totalData = signal(0);
...
private getTableData(pageOption: pageSelection): void {
  this.data.getPayment().subscribe((apiRes: apiResultFormat) => {
    ...
    const pagedTable: payment[] = [];
    const pagedCopy: payment[] = [];
    const serials: number[] = [];

    apiRes.data.forEach((res: payment, index: number) => {
...
ngOnInit() {
  this.loadTableData();
    ...
    if (this.router.url === this.AllRoutes.payments) {
