...
import { users, apiResultFormat } from '../../../../core/models/model';
import {
  pageSelection,
  PaginationService,
  tablePageSize,
...
export class Users {
  ...
  tableDataCopy = signal<users[]>([]);
  allData: WritableSignal<users[]> = signal([]);
  totalData = signal(0);
...
private getTableData(pageOption: pageSelection): void {
  this.data.getUsers().subscribe((apiRes: apiResultFormat) => {
    ...
    const pagedTable: users[] = [];
    const pagedCopy: users[] = [];
    const serials: number[] = [];

    apiRes.data.forEach((res: users, index: number) => {
...
ngOnInit() {
  this.loadTableData();
    ...
    if (this.router.url === this.AllRoutes.users) {
