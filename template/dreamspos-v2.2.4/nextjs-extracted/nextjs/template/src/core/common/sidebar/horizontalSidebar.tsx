                      ...
                      {!mainMenus.hasSubRoute && (
                        <li>
                          <Link
                            ...
                            target={('target' in mainMenus && (mainMenus as any).target) as React.HTMLAttributeAnchorTarget | undefined}
                            rel={
                              ('target' in mainMenus && (mainMenus as any).target === "_blank")
                                ? "noopener noreferrer"
                                : undefined
                            }
                          >
                        ...
                        <li className="submenu">
                          ...
                          >
                            {mainMenus.subRoutes?.map((subDropMenus, subIndex) => (
                              <li key={subIndex}>
                                <Link
                                  href={subDropMenus.route || "#"}
                                  className={isActiveSubMenu(subDropMenus) ? 'active' : ''}
                                  target={('target' in subDropMenus && (subDropMenus as any).target) as React.HTMLAttributeAnchorTarget | undefined}
                                  rel={
                                    ('target' in subDropMenus && (subDropMenus as any).target === "_blank")
                                      ? "noopener noreferrer"
                                      : undefined
                                  }
                                >
