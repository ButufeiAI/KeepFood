...
import { coupons, apiResultFormat } from '../../../../core/models/model';
import {
  pageSelection,
  PaginationService,
  tablePageSize,
...
export class Coupons {
  ...
  tableData = signal<coupons[]>([]);
  tableDataCopy = signal<coupons[]>([]);
  allData: WritableSignal<coupons[]> = signal([]);
  totalData = signal(0);
  ...
  loadTableData(): void {
     this.data.getCoupons().subscribe((res: apiResultFormat) => {
    // Add serial numbers
...
private getTableData(pageOption: pageSelection): void {
  this.data.getCoupons().subscribe((apiRes: apiResultFormat) => {
    ...
    const pagedTable: coupons[] = [];
    const pagedCopy: coupons[] = [];
    const serials: number[] = [];

    apiRes.data.forEach((res: coupons, index: number) => {
...
ngOnInit() {
  this.loadTableData();
    ...
    if (this.router.url === this.AllRoutes.coupons) {
...
showCoupon():void{
  window.alert("Coupons copied to Clipboard");
}
ngAfterViewInit() {
