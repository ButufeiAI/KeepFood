...
import { categories, apiResultFormat } from '../../../../core/models/model';
...
export class Categories {
  ...
  tableData = signal<categories[]>([]);
  tableDataCopy = signal<categories[]>([]);
  allData: WritableSignal<categories[]> = signal([]);
  totalData = signal(0);
...
private getTableData(pageOption: pageSelection): void {
  this.data.getCategories().subscribe((apiRes: apiResultFormat) => {
    ...
    const pagedTable: categories[] = [];
    const pagedCopy: categories[] = [];
    const serials: number[] = [];

    apiRes.data.forEach((res: categories, index: number) => {
...
ngOnInit() {
  this.loadTableData();
    ...
    if (this.router.url === this.AllRoutes.categories) {
